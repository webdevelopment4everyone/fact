...
import 'package:fact/widgets/line_items/LineItemInvoice.dart';
...
class InvoiceFormListWidget extends StatelessWidget {
  const InvoiceFormListWidget({super.key});

  @override
  Widget build(BuildContext context) {
    final taskList = Provider.of<InvoiceFormModelProvider>(context).itemsList;
    return taskList.length > 0
        ? ListView.builder(
            itemCount: taskList.length,
            //TODO: passthe line items
            itemBuilder: (context, index) {
              return LineItemInvoiceForm(taskList[index]);
            },
          )
        : LayoutBuilder(
